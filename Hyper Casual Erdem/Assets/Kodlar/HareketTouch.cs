...
public class HareketTouch : MonoBehaviour
{
    bool sagagit;
    bool solagit;
    void Start()
    ...
    void Update()
    {
        transform.Translate(0f, 0f, 0.003f);
        if (Input.touchCount > 0) //Dokunma sayýmýzý þartladýk.
        {

            Touch dokunma = Input.GetTouch(0); //0 sayýsý began methoduna eþit
            Vector3 sag = new Vector3(-4.4f, transform.position.y, transform.position.z);
            Vector3 sol = new Vector3(4.4f, transform.position.y, transform.position.z);
